...
type noOpReportDialer struct {
	dialer.Dialer
}

func (d *noOpReportDialer) ReportUnavailable(networkType *dialer.NetworkType, err error) {
}

func (d *noOpReportDialer) Property() *dialer.DialerProperty {
	return d.Dialer.Property()
}

func (d *noOpReportDialer) MustGetAlive(networkType *dialer.NetworkType) bool {
	return d.Dialer.MustGetAlive(networkType)
}
...
func (c *ControlPlane) RouteDialTcp(p *RouteDialParam) (conn netproxy.Conn, err error) {
	...
	if dialOption.Dialer != nil {
		dialOption.Dialer = &noOpReportDialer{Dialer: dialOption.Dialer}
	}
