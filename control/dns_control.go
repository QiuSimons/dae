...
func (c *DnsController) handleDNSRequest(
	...
	if RequestIndex == consts.DnsRequestOutboundIndex_AsIs {
		...
		upstream = &dns.Upstream{
			...
			Ip46:     netutils.FromAddr(req.realDst.Addr()),
		}
	} else {
		...
		if ResponseIndex.IsReserved() {
			if log.IsLevelEnabled(log.InfoLevel) {
				...
				case consts.DnsResponseOutboundIndex_Accept:
					log.WithFields(fields).Infof("[DNS] %v <-> %v", RefineSourceToShow(req.realSrc, req.realDst.Addr()), RefineAddrPortToShow(dialArgument.bestTarget))
