...
func Run(conf *config.Config, externGeoDataDirs []string) {
	...
	// New ControlPlane.
	c, err := newControlPlane(nil, conf, externGeoDataDirs, prometheus.NewRegistry())
	if err != nil {
		std.Fatalln(err)
	}
	...
	for {
		...
		case sig := <-sigs:
			...
			case syscall.SIGUSR1:
				...
				newC, err := newControlPlane(obj, newConf, externGeoDataDirs, prometheus.NewRegistry())
				if err != nil {
					reloadingErr = err
					std.Errorf("%+v", oops.Wrapf(err, "[Reload] Failed to reload; try to roll back configuration"))
					// Load last config back.
					newC, err = newControlPlane(obj, conf, externGeoDataDirs, prometheus.NewRegistry())
					if err != nil {
						sdnotify.Stopping()
						obj.Close()
						c.Close()
